...
ax.set_title("Sentiment Composition by Review Length",
             fontsize=15, fontweight="bold", color="#2D2D2D", pad=18)
ax.set_xticks(x)
ax.set_xticklabels(bin_summary["length_bin"].astype(str), fontsize=11, color="#333333")
...
# Add n labels under each pair of bars
for i, row in bin_summary.iterrows():
    ax.text(i, -0.06, f"n = {int(row['total']):,}", transform=ax.get_xaxis_transform(),
